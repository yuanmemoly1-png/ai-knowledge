...
**别处已经讲过的**：MCP 的机制在第 19.3 节已经讲透，多智能体怎么组织在第 18.7 节讲过，本节只讲跨系统通信这一层：能力怎么被发现、消息长什么样、失败怎么表达。
...
**这一节在全书里的位置**：Token 与计费口径在《Token与分词》和小白课计费那节讲过，路由这个模式在第 18.2 节讲过。本节只讲把资源当硬约束来决策这一层：预算怎么定、超标时砍什么。
...
**这一节的位置**：ReAct 循环在第 18.2 节讲过，反思作为独立模式在第 18.4 节讲过。本节只讲用多少推理时间换多少准确率这个权衡。
...
**不重复讲的部分**：幻觉的成因与识别在《幻觉Hallucination》里讲，调试与可观测性的工具在第 20.1 节讲。本节只讲在哪些位置设卡、卡住之后怎么处理、以及为什么单靠提示词不行。
...
### 什么时候不该用
两个反模式：护栏只加不减，规则越堆越多、没人清理过期的，维护成本超过收益。把默认值放反：书里的策略提示词写的是「仅在未发现明显违规时才默认为合规」，反过来写成「拿不准算违规」就是误杀正常流量。

护栏不是免费的：每次预筛都是一次额外调用，换成延迟和成本。更简单的替代是先用确定性手段吃掉大部分问题：正则、允许列表、schema 校验、权限范围：只在规则表达不了处才补模型判断。`,
    keypoints: [
...
离线一层，书里给了两种载体。测试文件是 JSON，表示单个简单的会话，适合开发期的单元测试，每轮记下用户 query、期望的工具调用轨迹和最终响应。评估集文件用于集成测试，包含多个很长的会话。

轨迹怎么比？书里列了几种匹配方式：精确匹配（必须与理想序列完全一致）、按序匹配（按序出现，允许额外步骤）、任意顺序匹配、精确度、召回、单工具使用。高风险场景要精确匹配。

线上盯四件事：延迟、token 用量、漂移（输入分布或环境变化导致退化）、行为异常。延迟和成本要落进持久存储，只打印控制台不够。

书里还对比了三种评估方法：人工准但慢，模型当裁判可扩展但漏中间步骤，自动化指标客观但覆盖不全。

为什么不能只用精确匹配？书里的例子：智能体答「The capital of France is Paris.」，参考答案是「Paris is the capital of France.」，字符串不相等，函数返回 0 分：语义一样却判错。所以要补上相似度、embedding、裁判模型这类指标。

评测集为什么会失效？书里明说环境不是静态的。评测集一旦冻结在发布那天，它就只测量那天的世界：流量分布变了、工具 API 改了、模型换了版本，它还在给满分。所以要靠线上抓到的真实失败案例定期回填。

**和相邻章节的分界**：目标设定与进展监控在第 19.4 节讲过，异常处理与恢复在第 20.1 节讲过。本节只讲测量本身：定义哪些指标、两层各测什么、评测集怎么不失效。
...
**别处已经讲过的**：怎么把目标拆成有依赖关系的步骤在第 18.6 节规划里讲过，怎么定义和跟踪目标在第 19.4 节讲过。本节只讲步骤数超过预算时，用什么标准排执行顺序、以及放弃什么。
...
### 展开讲
它不同于被动响应，也不同于在预定义解空间里做优化。它主动进入陌生领域、试没试过的方法、生成新知识，目标是发现「未知的未知」。
...
最该记住的一条：探索必须有预算上限和收敛条件。协同科学家靠「测试时计算扩展」控制算力投入，说明这是有方向的投入。Agent Laboratory 的研究角色带 max_steps 上限。工程上的写法是：给时间、token 或实验次数的硬上限；给一个「什么算找到了」的判定标准。再给一条无改进就停的规则。

**在全书里的位置**：智能体自我改进的机制在第 19.2 节讲过，多智能体怎么组织在第 18.7 节讲过。本节只讲目标不明确、解法空间没边界时，怎么让系统去搜、并且知道什么时候停。
...
解空间已明确、目标可量化的时候，探索不如直接优化：更快也更省。更简单的替代：人先圈定三到五个候选方向，让智能体只负责评估和排序。`,
    keypoints: [
